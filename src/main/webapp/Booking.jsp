    ...
    <div class="border" id="information">
      ...
      <div class="search-container">
        <form action="JiaogongSearchServlet" method="GET">
          <input type="text" id="search-input" placeholder="请输入关键词" name="jid">
          <button type="submit" id="search-button" >搜索</button>
        </form>
        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
        <a href="#" id="search-button1">添加</a>
      </div>
      <div id="data-list">
      <%
        // 获取用户列表
      ...
      <table border="1" style="width:1200px ;border-collapse: collapse;" >
        ...
        %>
        <tr>
          <td><%= booking.getJid() %></td>
          <td><%= booking.getSid() %></td>
          <td><%= booking.getName() %></td>
          <td><%= booking.getPhone() %></td>
          <td><%= booking.getEnterTime() %></td>
          <td><%= booking.getLeaveTime() %></td>
          <td><%= booking.getRoomType()%></td>
          <td>
            <a href="#?jid=<%= booking.getJid()%>">修改</a>
          </td>
          <td>
            <a href="#?jid=<%= booking.getJid()%>">删除</a>
          </td>
        </tr>
        <%
          }
